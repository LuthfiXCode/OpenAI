...
module.exports = maslent = async (client, m, chatUpdate, store) => {
    try {
        ...
        if (setting.autoAI) {
            ...
            console.log(chalk.red(chalk.bgBlack('< SYSTEM >')), color(argsLog, 'turquoise'), chalk.magenta('From'), chalk.green(pushname), chalk.yellow(`[𓂸 ]`))
            } else if (argsLog && m.isGroup) {
            
            console.log(chalk.red(chalk.bgBlack('< SYSTEM >')), color(argsLog, 'turquoise'), chalk.magenta('From'), chalk.green(pushname), chalk.yellow(`[𓂸]`), chalk.blueBright('IN'), chalk.green(groupName))
            }
